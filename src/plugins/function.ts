...
export interface FunctionPluginParam extends ObjectAction {
    ...
    /** 
     * 执行对象
     * 你可以选择目标来执行puppeteer函数，默认浏览器 >页面 >框架，但是如果 `框架frame` 被切换过 (frame.id不等于page.mainFrame().id) ，则框架的优先级大于页面
     * ****
     * you can select target to execute puppeteer function, default browser > page > frame, but if frame.id is not equals to page.mainFrame().id , the frame's priority is big than page
     */
